...
describe("SphereToken Smart Contract Tests", function() {
    this.beforeEach(async function() {
        ...
        const _Array = await ethers.getContractFactory("Array");
        array_library = await _Array.deploy();
        const SphereToken = await ethers.getContractFactory("SphereToken", {
            libraries: {
                Array: array_library.address
            }
        });
        sphere_token = await SphereToken.deploy();
    ...
    it("Simple Test for Multi Token", async function() {
        try {
            ...
            */
            ...
            const tx5 = await sphere_token.connect(account1).addTokenType(10, 100, "https://garaokay/dfd/ppgh/fllowow/son");
            /*
            tokenId is 6.
